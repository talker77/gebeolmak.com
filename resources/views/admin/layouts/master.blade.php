...
<input id="count-forum-pending" type="hidden" data-item="label_forum-pending" value="{{ $counts['forum']['pending'] }}">
<input id="count-forum-comment-pending" type="hidden" data-item="forum-comment-pending" value="{{ $counts['forum']['pending_comments'] }}">
<div class="wrapper">
...
<script>
    // constants
    const IS_ADMIN = "{{ loggedAdminUser()->isSuperAdmin() ? 1: 0 }}";
    const CONSTANTS = {
        MULTI_LANGUAGE: "{{ config('admin.multi_lang') ? 1 : 0 }}",
    }
</script>
@yield('footer')
</body>
